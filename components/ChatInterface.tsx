    ...
    <div className="flex flex-col h-full bg-ocean-900 relative pt-safe-top">
      ...
      <div className="absolute top-0 left-0 w-full z-20 bg-gradient-to-b from-ocean-900/90 to-transparent backdrop-blur-[2px] pt-[calc(env(safe-area-inset-top)+10px)] pb-2 px-4 flex items-center justify-between pointer-events-none">
        <div className="flex items-center gap-3 overflow-hidden pointer-events-auto">
             {onBack && (
                <button onClick={onBack} className="p-2 -ml-2 text-white/80 hover:text-gold-500 transition-colors bg-black/20 rounded-full border border-white/5 backdrop-blur-md">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
                </button>
             )}
             {activeProductContext ? (
                 <div className="flex items-center gap-2 overflow-hidden bg-black/20 backdrop-blur-md rounded-full pr-4 border border-white/5">
                     <div className="w-8 h-8 rounded-full overflow-hidden border border-gold-500/30 flex-shrink-0 ml-0.5 my-0.5">
        ...
        <div className="flex items-center gap-2 flex-shrink-0 pointer-events-auto">
            {onClearChat && (
                <button onClick={onClearChat} className="w-9 h-9 rounded-full bg-black/20 border border-white/5 flex items-center justify-center hover:bg-white/10 text-gray-400 backdrop-blur-md transition-colors">
                     <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg>
                </button>
            )}
            {onOpenCart && (
                <button onClick={onOpenCart} className="w-9 h-9 rounded-full bg-black/20 border border-white/5 flex items-center justify-center hover:bg-white/10 text-gold-500 relative backdrop-blur-md transition-colors">
      ...
      <div className="absolute bottom-6 left-4 right-4 z-40">
        ...
        {showTools && (
            <div className="mb-4 bg-ocean-900/95 backdrop-blur-xl border border-ocean-700/50 rounded-2xl p-6 grid grid-cols-4 gap-6 animate-slide-in-up shadow-2xl relative z-50">
                <button onClick={() => fileInputRef.current?.click()} className="flex flex-col items-center gap-2 text-gray-300 hover:text-white group">
                    <div className="w-14 h-14 bg-ocean-800 rounded-2xl flex items-center justify-center border border-white/5 group-hover:border-gold-500/30 group-hover:bg-gold-500/10 transition-all shadow-inner">
